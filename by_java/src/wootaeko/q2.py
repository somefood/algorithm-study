...
def solution(log):
    ...
    for i in range(0, len(log), 2):
        stop_hour, stop_min = map(int, log[i+1].split(":"))
        start_hour, start_min = map(int, log[i].split(":"))
        stop_second = stop_hour * 3600 + stop_min * 60
        start_second = start_hour * 3600 + start_min * 60

        result_second = stop_second - start_second

        if result_second < 300:
            pass
        elif result_second > 6300:
            result += 6300
        else:
